...
function sortByValues(dictionary) {
  ...
  const convertDictionary = [...dictionary];
  convertDictionary.sort((a,b) => b[1] - a[1]);
  const sortedDictionary = new Map(convertDictionary);
...
function convertSetToDictionary(set) {
 ...
 const firstCode = element.charCodeAt(0);
    dictionary.set(element, firstCode);
  }
  return dictionary;
}
